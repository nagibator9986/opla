...
def _freeform_chat(session: ChatSession, user_content: str):
    ...
    if is_registered:
        ...
        system_prompt += (
            ...
            "8. ФОРМАТ ВЫВОДА: пиши обычным текстом без markdown. "
            "НЕ используй **звёздочки** для выделения, НЕ используй заголовки "
            "решёткой #, НЕ используй markdown-списки со звёздочками. "
            "Если нужно выделить — пиши через двоеточие или просто abzac. "
            "Ссылки оформляй прямо: «/tariffs», «/cabinet», «/cases» — фронт "
            "автоматически сделает их кликабельными. НЕ оборачивай ссылки в "
            "квадратные скобки [текст](url) — пиши прямой путь."
        )
    history = [
