...
class AuditReportAdmin(ModelAdmin):
    """Технический интерфейс к AuditReport.

    Скрыт из бокового меню (`has_module_permission = False`) — основная
    работа админа теперь идёт со страницы заявки. Прямой URL остаётся
    доступным, чтобы старые ссылки не сломались.
    """

    def has_module_permission(self, request):
        # Скрываем из app_list в меню. Прямой URL работает.
        return False
    ...
    def approve_and_send(self, request, object_id):
        ...
        if not ok:
            messages.error(request, err or "Ошибка утверждения.")
        else:
            messages.success(
                request,
