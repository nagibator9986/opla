...
class QuestionnaireTemplate(TimestampedModel):
    ...
    intro_text = models.TextField(
        ...
        help_text=(
            "Вводный текст в чате ПЕРЕД первым вопросом анкеты. "
            "Поддерживает плейсхолдеры {{name}}, {{company}}, {{total}} — "
            "подставятся автоматически. Это именно тот текст, который "
            "клиент видит на скриншоте «Сейчас я задам вам N вопросов…»."
        ),
    )
    completion_text = models.TextField(
        ...
        help_text=(
            "Текст в чате ПОСЛЕ последнего вопроса (благодарность + ETA доставки). "
            "Плейсхолдеры {{name}}, {{company}}, {{total}}."
        ),
    )
