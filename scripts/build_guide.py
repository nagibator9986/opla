...
def cover_page() -> list:
    return [
        ...
        key_value_table([
            ...
            ("Версия документа", "v2.0 · 27 мая 2026 (обновлено после полного аудита прода)"),
        ]),
        PageBreak(),
    ]


def section_release_notes_may2026() -> list:
    """Что обновлено в релизе от 27.05.2026 — отдельный раздел.

    Покрывает 6 правок по требованиям клиента: универсальная анкета,
    администрируемый футер, антибелиберда, улучшенная админка.
    """
    return [
        p("00. Что обновлено 27 мая 2026", "h1"),
        p(
            "Этот релиз закрыл 6 пунктов из обратной связи владельца. "
            "Все правки уже на проде, ничего вручную настраивать не нужно — "
            "ниже только справка «где что теперь».",
        ),

        p("1) Анкета: 9 → 55 вопросов", "h2"),
        p(
            "<b>До:</b> при оформлении заявки клиенту показывалась "
            "устаревшая 9-вопросная демо-анкета (на скриншоте — «Сейчас "
            "я задам вам 9 вопросов»).",
        ),
        p(
            "<b>Сейчас:</b> единственная активная анкета — <b>«Digital "
            "Baqsylyq»</b>, 55 вопросов (3 этапа + 2 ветки по роли). "
            "Менеджер видит 25 вопросов, владелец / топ-менеджер — 38. "
            "Все 5 старых демо-шаблонов деактивированы (не удалены — "
            "сохранены для исторических заявок).",
        ),
        p(
            "<b>Где править вопросы:</b> /admin/industries/question/ — "
            "редактирование текста, типа поля, условной видимости. "
            "<b>Где править intro/completion-тексты чата:</b> "
            "/admin/industries/questionnairetemplate/ — поля "
            "<code>intro_text</code> (показывается перед первым "
            "вопросом — это тот самый «Сейчас задам N вопросов») и "
            "<code>completion_text</code> (после последнего — "
            "«Спасибо, … анкета завершена»).",
        ),

        p("2) Тексты футера — все в админке", "h2"),
        p(
            "<b>До:</b> только 4 строки футера правились (описание "
            "под лого, ИП, БИН, адрес). Заголовки «Продукт», «Контакты», "
            "«Документы», ссылки, телефон, email, текст про CloudPayments — "
            "были захардкожены.",
        ),
        p(
            "<b>Сейчас:</b> все 17 текстов футера в "
            "/admin/content/contentblock/ под секцией <b>«Футер»</b>. "
            "Ключи: <code>footer_col_product_heading</code>, "
            "<code>footer_link_tariffs/cases/articles/faq</code>, "
            "<code>footer_col_contacts_heading</code>, "
            "<code>footer_contact_email</code>, "
            "<code>footer_contact_whatsapp</code>, "
            "<code>footer_contact_whatsapp_url</code>, "
            "<code>footer_col_documents_heading</code>, "
            "<code>footer_link_offer/privacy/refund</code>, "
            "<code>footer_legal_heading</code>, "
            "<code>footer_security_heading</code>, "
            "<code>footer_security_line1/2</code>, "
            "<code>footer_copyright_suffix</code>.",
        ),

        p("3) Приветствие чата Baqsy AI", "h2"),
        p(
            "Текст «Здравствуйте, &lt;имя&gt;! Я Baqsy AI — ваш "
            "персональный помощник…» — это ContentBlock с ключом "
            "<b>chat_greeting_authed</b>. Открывается в "
            "/admin/content/contentblock/ → секция «Чат / AI». "
            "Поддерживает плейсхолдеры <code>{{name}}</code> и "
            "<code>{{company}}</code> — они подставляются "
            "автоматически из профиля клиента.",
        ),

        p("4) Защита от «белиберды» в ответах", "h2"),
        p(
            "Раньше AI-бот принимал любой ответ, включая случайный "
            "набор символов («куруерукреу»). Теперь работают "
            "5 эвристик на TEXT и LONGTEXT вопросы:",
        ),
        bullet("Минимальная длина для развёрнутого ответа — 15 символов"),
        bullet("Защита от длинных повторов одного символа («ааааа»)"),
        bullet("Минимум 2 слова для глубоких вопросов аудита"),
        bullet("Уникальность букв ≥ 22% (отлавливает «куруерукреу куруерукреу»)"),
        bullet("Адекватное соотношение гласных/согласных (15–85%)"),
        p(
            "При попадании в эвристику бот пишет понятное сообщение "
            "(«Похоже на случайный набор символов — опишите по сути "
            "вопроса») и повторяет тот же вопрос. Короткие легитимные "
            "ответы вроде «ИП», «Алматы», «5 лет» — пропускаются.",
        ),

        p("5) Удобный просмотр ответов в админке", "h2"),
        p(
            "Полностью переработан экран заявки "
            "/admin/submissions/submission/&lt;id&gt;/. Теперь сверху — "
            "карточный read-only блок <b>«Ответы клиента (по этапам)»</b>:",
        ),
        bullet("Ответы сгруппированы по этапам (Паспорт / Роль / Аудит)"),
        bullet("Каждый вопрос подсвечен, ответ в отдельном блоке"),
        bullet("Цветной бейдж статуса в списке заявок (создана / оплачена / завершена / доставлена)"),
        bullet("Прогресс «отвечено / всего» в списке"),
        p("<b>Три кнопки сверху:</b>", "body_strong"),
        bullet(
            "<b>📋 Скопировать все ответы</b> — формат «В: вопрос / "
            "О: ответ», готов для вставки в любой документ или CRM"
        ),
        bullet(
            "<b>📊 Скачать CSV</b> — UTF-8 с BOM, открывается в Excel "
            "без проблем с кириллицей. Колонки: Этап, №, Вопрос, Тип, "
            "Ответ, Дата ответа. Имя файла = "
            "<code>baqsy_answers_&lt;Компания&gt;_&lt;Имя&gt;.csv</code>"
        ),
        bullet(
            "<b>🖨️ Распечатать</b> — открывает системный диалог печати, "
            "формат идеален для бумажной работы эксперта"
        ),
        p(
            "Старый табличный inline сохранён, но свёрнут — кому "
            "привычнее так, может открыть.",
            "small",
        ),

        p("6) Помощь админу прямо в формах", "h2"),
        p(
            "В Django-админке у полей <code>intro_text</code>, "
            "<code>completion_text</code> и ContentBlock'ов теперь "
            "развёрнутые подсказки (help_text), которые явно говорят: "
            "«это именно тот текст, что клиент видит на скриншоте». "
            "В списке ContentBlock — секции «Футер», «Чат / AI», "
            "«Реквизиты», «Hero», «Метод», «Тарифы», «Кейсы», «FAQ», "
            "«Статьи» (для удобной навигации между 62+ блоками).",
        ),
...
def section_admin_flow() -> list:
    return [
        ...
        # Контент
        step_card(
            "АДМИН · 02", "Тексты лендинга, футера и чата",
            "/admin/content/contentblock/",
            [
                p(
                    "62 активных блока: всё текстовое содержимое сайта — "
                    "Hero, метод, тарифы, FAQ, статьи, <b>весь футер</b> "
                    "(17 ключей), реквизиты, приветствие AI-чата, даты "
                    "редакций оферты.",
                ),
                p(
                    "Плашки секций для удобной навигации: <b>Hero</b>, "
                    "<b>Метод</b>, <b>Тарифы</b>, <b>Кейсы</b>, <b>FAQ</b>, "
                    "<b>Статьи</b>, <b>Футер</b>, <b>Реквизиты</b>, "
                    "<b>Чат / AI</b>. Превью текста, кнопка «✏️ Изменить» "
                    "в каждой строке, фильтр по секциям и активности.",
                ),
                p(
                    "<b>Шпаргалка — где какой текст:</b>",
                    "body_strong",
                ),
                bullet("Приветствие Baqsy AI («Здравствуйте, …! Я Baqsy AI…») — ключ <b>chat_greeting_authed</b>"),
                bullet("Описание под лого в футере — <b>footer_description</b>"),
                bullet("Email/WhatsApp в футере — <b>footer_contact_email</b>, <b>footer_contact_whatsapp</b>"),
                bullet("Заголовки колонок футера — <b>footer_col_*_heading</b>"),
                bullet("Реквизиты ИП — <b>legal_company_name/bin/address</b>"),
                bullet("FAQ — <b>faq_N_q</b> / <b>faq_N_a</b> (N = 1..5)"),
                bullet("Hero-секция — все ключи на <b>hero_*</b>"),
                p(
        ...
        # Приём заказа
        step_card(
            "АДМИН · 08", "Заказы и анкеты — карточный просмотр + экспорт",
            "/admin/submissions/submission/",
            [
                p(
                    "<b>Список заявок:</b> цветные бейджи статусов "
                    "(создана / оплачена / в работе / завершена / "
                    "доставлена), прогресс «отвечено/всего», фильтры "
                    "по статусу, тарифу, отрасли. Поиск по имени / "
                    "компании клиента или ID заявки.",
                ),
                p("<b>Внутри карточки заявки</b> — два просмотра:", "body_strong"),
                bullet(
                    "<b>«Ответы клиента (по этапам)»</b> — карточный read-only "
                    "вид: каждый этап выделен заголовком, вопрос на отдельной "
                    "строке, ответ в подсвеченном блоке. Идеально для чтения "
                    "глазами эксперта."
                ),
                bullet(
                    "<b>«Ответы клиента (таблица)»</b> — сворачиваемый "
                    "классический Django inline, для тех, кому привычнее "
                    "табличный формат."
                ),
                p("<b>Три кнопки сверху карточки ответов:</b>", "body_strong"),
                bullet(
                    "<b>📋 Скопировать все ответы</b> — кладёт в буфер обмена "
                    "форматированный текст «В: вопрос / О: ответ» с разделителями "
                    "этапов. Готово для вставки в Notion / Google Docs / любой "
                    "редактор."
                ),
                bullet(
                    "<b>📊 Скачать CSV</b> — Excel-совместимый файл, UTF-8 с BOM, "
                    "разделитель «;». Колонки: Этап, №, Вопрос, Тип, Ответ, "
                    "Дата ответа. Имя файла = "
                    "<code>baqsy_answers_&lt;Компания&gt;_&lt;Имя&gt;.csv</code>."
                ),
                bullet(
                    "<b>🖨️ Распечатать</b> — открывает системный диалог печати, "
                    "корректно форматирует все ответы на бумагу."
                ),
                p(
                    "Когда статус заказа = <i>completed</i> (анкета "
                    "пройдена), снизу появляется inline-форма "
                    "<b>AuditReport</b> — там эксперт пишет текст аудита и "
                    "нажимает «Подтвердить и отправить PDF».",
                ),
            ],
        ),
...
def section_technical() -> list:
    return [
        ...
        Paragraph(
            ...
            "<br/>"
            "# Подсыпать стартовые данные (идемпотентно)<br/>"
            "docker compose -f docker/docker-compose.prod.yml exec -T web python manage.py seed_content<br/>"
            ...
            "<br/>"
            "# Гарантировать что Baqsylyq — единственная активная анкета<br/>"
            "docker compose -f docker/docker-compose.prod.yml exec -T web python manage.py activate_baqsylyq<br/>"
            "<br/>"
...
def section_checklist() -> list:
    return [
        ...
        Spacer(1, 8),
        p("Контент", "h3"),
        bullet("✓ 62 активных текстовых блока (Hero / Метод / Тарифы / FAQ / Статьи / <b>весь футер</b> / Реквизиты / Чат)"),
        bullet("✓ 12 параметров аудита с промптами"),
        bullet("✓ Анкета «Digital Baqsylyq» — 55 вопросов (25 для менеджера, 38 для владельца / топа)"),
        bullet("✓ Демо-шаблоны (5 шт.) деактивированы — новые заявки идут только в Baqsylyq"),
        bullet("✓ 2 демо-кейса (нужно заменить на реальные)"),
        bullet("☐ Привязать вопросы к 12 параметрам (через bulk-action в /admin/industries/question/)"),
...
def build():
    ...
    story: list = []
    story += cover_page()
    story += section_release_notes_may2026()
